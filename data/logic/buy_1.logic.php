   ...
   /**
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
       ...
       // $new_array['goods_price'] = $goods_info['goods_price'];
		...
		//批发商品
		$new_array['is_pf'] = $goods_info['is_pf'];
		$new_array['pf_price1'] = $goods_info['pf_price1'];
		$new_array['pf_price2'] = $goods_info['pf_price2'];
		$new_array['pf_price3'] = $goods_info['pf_price3'];
		$new_array['pf_num1'] = $goods_info['pf_num1'];
		$new_array['pf_num2'] = $goods_info['pf_num2'];
		$new_array['pf_num3'] = $goods_info['pf_num3'];
		...
		if($new_array['is_pf']==1 && $new_array['goods_num'] >= $new_array['pf_num1']){ //一级判断商品是否属于批发商品并大于一级购买量
		if($new_array['pf_num3']>0){ //第三级商品是否存在
        if($new_array['pf_num2'] > $new_array['goods_num'] && $new_array['goods_num'] >= $new_array['pf_num1']){	
		if(!empty($new_array['vip_m_info'])){ //当商品为允许会员折扣的时候
		$new_array['goods_old_price']= $goods_info['pf_price1'];
		$new_array['goods_price'] = ncPriceFormat($goods_info['pf_price1'] * $vip_m_commit/100);
		}else{
			$new_array['goods_price'] = $goods_info['pf_price1'];		
		}		
				
		}else if($new_array['pf_num3'] > $new_array['goods_num'] && $new_array['goods_num']>=$new_array['pf_num2']){
			
		if(!empty($new_array['vip_m_info'])){ //当商品为允许会员折扣的时候
		$new_array['goods_old_price']= $goods_info['pf_price2'];
		$new_array['goods_price'] = ncPriceFormat($goods_info['pf_price2'] * $vip_m_commit/100);
		}else{
			$new_array['goods_price'] = $goods_info['pf_price2'];		
		}
				
			}else if($new_array['goods_num']>=$new_array['pf_num3']){
		
			
		if(!empty($new_array['vip_m_info'])){ //当商品为允许会员折扣的时候
		$new_array['goods_old_price']= $goods_info['pf_price3'];
		$new_array['goods_price'] = ncPriceFormat($goods_info['pf_price3'] * $vip_m_commit/100);
		}else{
			$new_array['goods_price'] = $goods_info['pf_price3'];		
		}	
		
		
			}
        }else if($new_array['pf_num2']>0 && 0>=$new_array['pf_num3']){ //第二级商品是否存在
        if($new_array['pf_num2'] > $new_array['goods_num'] && $new_array['goods_num'] >= $new_array['pf_num1']){
		...
		$new_array['goods_price'] = ncPriceFormat($goods_info['pf_price1'] * $vip_m_commit/100);
			...
			}else if($new_array['goods_num']>=$new_array['pf_num2']){
		...
		if(!empty($new_array['vip_m_info'])){ //当商品为允许会员折扣的时候
		$new_array['goods_old_price']= $goods_info['goods_price'];
		$new_array['goods_price'] = ncPriceFormat($goods_info['goods_price'] * $vip_m_commit/100);
		}else{
		$new_array['goods_price'] = $goods_info['goods_price'];
		}
		}
    ...
    private function _getOnlineCartList($cart_list) {
		...
		if($goods_online_info['is_pf']==1 && $cart_info['goods_num'] >= $goods_online_info['pf_num1']){
			if($goods_online_info['pf_num3']>0){ //二级判断是否启用了第三阶梯
				if($cart_info['goods_num'] >= $goods_online_info['pf_num1'] && $goods_online_info['pf_num2']>=$cart_info['goods_num']){//当所选数量为一级阶梯价格
					if(!empty($cart_list[$key]['vip_mt_info'])){ //当商品为允许会员折扣的时候
				...
				}else if($cart_info['goods_num'] >= $goods_online_info['pf_num2'] && $goods_online_info['pf_num3']>=$cart_info['goods_num']){//当所选数量为一级阶梯价
					if(!empty($cart_list[$key]['vip_mt_info'])){ //当商品为允许会员折扣的时候
				...
				}else if($cart_info['goods_num'] >= $goods_online_info['pf_num3']){//当所选数量为一级阶梯价
					if(!empty($cart_list[$key]['vip_mt_info'])){ //当商品为允许会员折扣的时候
			...
			}else if($goods_online_info['pf_num2']>0) {//二级判断是否启用了第二阶梯
				if($cart_info['goods_num'] >= $goods_online_info['pf_num1'] && $goods_online_info['pf_num2']>=$cart_info['goods_num']){//当所选数量为一级阶梯价格
					if(!empty($cart_list[$key]['vip_mt_info'])){ //当商品为允许会员折扣的时候
		              $cart_list[$key]['goods_old_price']= $goods_online_info['pf_price1'];
		              $cart_list[$key]['goods_price'] = ncPriceFormat($goods_online_info['pf_price1'] * $vip_m_commits/100);
			      ...
			      $cart_list[$key]['goods_price'] = $goods_online_info['pf_price1'];}	
				}else if($cart_info['goods_num'] >= $goods_online_info['pf_num2']){//当所选数量为一级阶梯价
					if(!empty($cart_list[$key]['vip_mt_info'])){ //当商品为允许会员折扣的时候
		              $cart_list[$key]['goods_old_price']= $goods_online_info['pf_price2'];
		              $cart_list[$key]['goods_price'] = ncPriceFormat($goods_online_info['pf_price2'] * $vip_m_commits/100);
		           }else{
